...
let i = 0;  // Aggiungi il contatore per verificare quante volte viene chiamata la funzione
async function getIdFromName(user1, user2) {
  ...
  if (!user1 || !user2) {
    ...
    return null;  // Se i parametri non sono definiti, ritorna null
  }
      ...
      if (err || !chat) {
        db.get('SELECT id FROM chats WHERE name = ?', [chatName2], (err, chat) => {
          if (err || !chat) {
            ...
            createChat(user1, user2).then((newChatId) => resolve(newChatId));  // Crea la chat e risolvi con l'ID creato
          } else {
            console.log(`Chat trovata: ${chatName2}`);
            resolve(chat.id); // Restituisci l'ID della chat trovata
          }
        });
      } else {
...
async function saveMessageToDb(chatName, username, message) {
  ...
  try {
    const chatId = await getIdFromName(username, chatName);  // Verifica che la chat esista
    ...
    db.run('INSERT INTO messages (chat_id, username, message) VALUES (?, ?, ?)', [chatId, username, message], (err) => {
      if (err) {
        ...
        messagesCache.delete(chatId); // Invalidate cache
      }
    });
  } catch (err) {
